...
test("target editing persists after refresh on desktop and mobile", async ({
  page,
  browserName
}, testInfo) => {
  await loadDashboard(page);
  ...
  const readingMinutesFits = await reloadedTargetDashboard
    .getByLabel("Reading minutes")
    .evaluate((element) => {
      const box = element.getBoundingClientRect();
...
test("daily check-in reaches all clear without desktop or mobile overflow", async ({
  page
}, testInfo) => {
  await loadDashboard(page);
  await openView(page, "Check-in");
  ...
  await expect(checkIn.getByText("All Clear")).toHaveCount(0);
  ...
  await openView(page, "Check-in");
  await expect(checkIn.getByText("Study time targets pending")).toHaveCount(0);
  ...
  const checkInRowsFitViewport = await checkIn
...
test("manual external time entry records selected section on desktop and mobile", async ({
  page
}, testInfo) => {
  await loadDashboard(page);
  await openView(page, "Timer");
